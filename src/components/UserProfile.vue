...
<template>
  <div
    v-if="userProfile"
    class="bg-white rounded-[15px] p-6 md:p-12 dark:bg-[#1E2A47]"
  >
    <div class="flex gap-[37px]">
      ...
      <div class="flex md:justify-between md:flex-row flex-col w-full">
        <div>
          <h1
            class="font-bold lg:text-[26px] text-4 text-[#2B3442] dark:text-white"
          >
            {{ userProfile.name }}
          </h1>
          <h2 class="text-base text-[#0079FF]">{{ userProfile.login }}</h2>
        </div>
        <p class="text-[#697C9A] lg:text-[15px] text-[13px] dark:text-white">
    ...
    <div class="lg:pl-[148px] mt-8 flex flex-col gap-8 md:pl-0 md:mt-6">
      <p
        class="lg:text-[15px] text-[13px] leading-[25px] text-[#4B6A9B] dark:text-white"
      >
        {{ userProfile.bio || "This profile has no Bio" }}
      </p>

      <ul
        class="bg-[#F6F8FF] w-full flex gap-8 md:gap-[99px] py-[15px] px-8 rounded-[10px] dark:bg-[#141D2F]"
      >
        <li class="text-[11px] md:text-[13px] text-[#4B6A9B] dark:text-white">
          Repos <br />
          <span
            class="text-[16px] md:text-[22px] font-bold text-[#2B3442] dark:text-white"
            >{{ userProfile.public_repos }}</span
          >
        </li>
        <li class="text-[11px] md:text-[13px] text-[#4B6A9B] dark:text-white">
          Followers <br /><span
      ...
      <ul class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-y-[21px]">
        <li class="flex items-center gap-[19.2px] dark:text-white">
          <img src="../assets/location-icon.svg" alt="location icon" />
          <span
            class="text-[#4B6A9B] text-[13px] md:text-[15px] dark:text-white"
            >{{ userProfile.location || "Not available" }}</span
          >
        </li>
        <li class="flex items-center gap-[19.2px] cursor-pointer">
          ...
          <span
            class="text-[#4B6A9B] text-[13px] md:text-[15px] dark:text-white"
            >{{ userProfile.twitter_username || "Not available" }}</span
          >
        </li>
        <li class="flex items-center gap-[19.2px] cursor-pointer">
        ...
        <li class="flex items-center gap-[19.2px]">
          ...
          <span
            class="text-[#4B6A9B] text-[13px] md:text-[15px] dark:text-white"
            >{{ userProfile.company || "Not available" }}</span
          >
        </li>
      </ul>
    </div>
